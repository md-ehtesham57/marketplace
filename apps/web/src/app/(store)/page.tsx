...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg key={star} className={"w-3.5 h-3.5 " + (star <= Math.floor(rating) ? "text-amber-400" : "text-slate-300")} fill="currentColor" viewBox="0 0 20 20">
    ...
    <div className="bg-slate-50 min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
          {categories.map((cat) => (
            <a key={cat.name} href={"/categories/" + cat.name.toLowerCase().replace(" & ", "-")}
            ...
            <a key={product.id} href={"/products/" + product.id} className="group">
              <Card padding="none" className="overflow-hidden hover:shadow-lg transition-shadow duration-200">
                <div className="relative bg-gradient-to-br from-slate-100 to-slate-200 h-48 flex items-center justify-center">
                  ...
                  {product.badge && (
                    <div className="absolute top-3 left-3">
                      <Badge variant={badgeVariant[product.badge] ?? "default"}>{product.badge}</Badge>
                    </div>
                  )}
                ...
                <div className="p-4">
                  ...
                  <h3 className="text-sm font-semibold text-slate-800 mb-2 line-clamp-2 group-hover:text-sky-600 transition-colors">{product.name}</h3>
                  <StarRating rating={product.rating} />
                  <p className="text-xs text-slate-400 mt-0.5">{product.reviews.toLocaleString()} reviews</p>
                  <div className="flex items-center justify-between mt-3">
                    <div>
                      <span className="text-lg font-bold text-slate-900">RS {product.price.toLocaleString()}</span>
